...
- **Aba separada (janela nova, `abrirPedidoAba`)** — abre o pedido em uma página independente, tabela única com campos de quantidade/sugestão editáveis ao vivo e recálculo automático do total geral. Além de **💾 Salvar Pedido** e **🖨️ Imprimir**, a janela tem botões **⬇ Excel (.csv)** e **🖨 PDF** (iguais aos da RHS/SEVEN): eles primeiro sincronizam as edições de qtd/sugestão de volta para `window.opener.pedido`, depois montam o conteúdo com os builders da janela principal (`window.opener.buildCSV`/`buildPDFHtml`) e **fazem o download dentro da própria janela do pedido** (âncora anexada ao DOM, `revokeObjectURL` adiado). Importante: o download precisa acontecer no documento da janela do pedido — chamar diretamente `window.opener.exportarExcelDireto()` fazia o arquivo ser gerado na aba principal (em segundo plano) e o navegador não salvava nada visível.
